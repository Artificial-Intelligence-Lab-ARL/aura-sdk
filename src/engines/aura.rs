...
#[cfg(feature = "aura-engine")]
impl AuraEngine {
    pub fn new(model_path: &Path, soc_model: &str) -> anyhow::Result<Self> {
        ...
        let session_wrapper =
            OrtSession::new(model_path, &backend_path, &output_bin_str, soc_model)?;
        let session = session_wrapper.session;
    ...
    pub fn query(
        ...
        for step in 0..max_tokens {
            let input_descriptors: Vec<(String, ort::value::ValueType)> = self
                .session
                .inputs()
                .iter()
            ...
            for (name, ty) in &input_descriptors {
                ...
                if lowered.contains("input_ids") {
                    ...
                    } = ty
                    {
                        ...
                        inputs.push((
                            name.clone(),
                            DynTensor::new(&ort::memory::Allocator::default(), *el_ty, res_shape)?
                                .into_dyn(),
                        ));
                    }
                }
            ...
            if let Ok(token_str) = self.tokenizer.decode(&[next_token as u32], true) {
                callback(&token_str);
            }
        }
        Ok(())
